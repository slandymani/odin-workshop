...
// CmdRequestRandomNumbersData creates and broadcast a RandomNumbers request transaction
func CmdRequestRandomNumbersData() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "random-numbers-data [oracle-script-id] [requested-validator-count] [sufficient-validator-count]",
		Short: "Make a new RandomNumbers query request via an existing BandChain oracle script",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			length, err := cmd.Flags().GetInt64(flagLength)
			if err != nil {
				return err
			}

			calldata := &types.RandomNumbersCallData{
				Length: length,
			}
			...
			msg := types.NewMsgRandomNumbersData(
	...
	cmd.Flags().Int64(flagLength, 8, "Multiplier used in calling the oracle script")
